...
class OccupancyGripMap:
    def __init__(self):
        ...
        self.map_odom_timer = rospy.Timer(
            rospy.Duration(0.1), self.broadcast_map_odom)
    ...
    def scan_cb(self, scan_msg):
        ...
        try:
            odom_tf = self.tf_buffer.lookup_transform(
                'odom', 'base_scan', rospy.Time(0)).transform
        except tf2_ros.TransformException:
            rospy.logwarn(
                'Pose from odom lookup failed. Using origin as odom.')
            odom_tf = convert_pose_to_tf(self.map_msg.info.origin)

        # get odom in frame of map
        odom_map_tf = tf_mat_to_tf(
            np.linalg.inv(tf_to_tf_mat(convert_pose_to_tf(
                self.map_msg.info.origin))).dot(tf_to_tf_mat(odom_tf))
        )
        odom_map = np.zeros(3)
        ...
        for i, r in enumerate(ranges):
            if (scan_msg.range_min < r < scan_msg.range_max):
                ...
                world_angle = odom_map[2] + SCAN_DOWNSAMPLE * \
